...
void setup() {
  ...
  graphics = createGraphics(120, 120, P3D);
...
void draw() {
  ...
  if (init_com==1 && graph_on==1) {
    ...
    if ((toggleWriteServo || toggleWriteServoLive )&& !toggleRead) {   // MSP_SET_SERVO_CONF
      ...
      if (multiType == AIRPLANE || multiType == PPM_TO_SERVO || multiType == HELI_120_CCPM || multiType == HELI_90_DEG) {
        for ( i=0; i<8; i++) {
          int q= (int)ServoSliderMIN[i].getValue();
          payload.add(char (q % 256) );
          payload.add(char (q / 256)  ); // Min
          q= (int)ServoSliderMAX[i].getValue();
          payload.add(char (q % 256) );
          payload.add(char (q / 256)  ); // Max
          q= (int)(ServoSliderC[i].getValue());
          payload.add(char (q % 256) );
      ...
      //******************************************************************************************
        ...
        for ( i=0; i<8; i++) {
          servoRATE[i]=round(RateSlider[i].getValue());
          payload.add(char(servoRATE[i]));
        }// servoRATE
        for ( i=0; i<8; i++) {
          int q= (int)(ServoSliderC[i].getValue())+512;
          payload.add(char (q % 256) );
          payload.add(char (q / 256)  );
        } // Servo centers...
        for ( i=0; i<8; i++) {
    ...
    if (toggleWriteWing || toggleWriteWingLive) { // MSP_SET_SERVO_CONF
      ...
      if (toggleGimbal || toggleTrigger) {  // MSP_SET_SERVO_CONF
        ...
        ServoMIN[0]=(int)GimbalSlider[0].getValue();
        ServoMIN[1]=(int)GimbalSlider[4].getValue();
        ServoMIN[2]=(int)GimbalSlider[8].getValue();
        ServoMAX[0]=(int)GimbalSlider[1].getValue();
        ServoMAX[1]=(int)GimbalSlider[5].getValue();
        ServoMAX[2]=(int)GimbalSlider[9].getValue();
        ServoMID[0]=(int)GimbalSlider[2].getValue();
        ServoMID[1]=(int)GimbalSlider[6].getValue();
        ServoMID[2]=(int)GimbalSlider[10].getValue();
        servoRATE[0]=(int)GimbalSlider[3].getValue();
        servoRATE[1]=(int)GimbalSlider[7].getValue();
        servoRATE[2]=(int)GimbalSlider[11].getValue();
    ...
    if (gimbal && gimbalConfig ) {
      if ((int)GimbalSlider[11].getValue()==1) {
      ...
      for ( i=2; i<11; i+=4) {
        ...
        if (GimbalSlider[i].getValue() < ValLow ) {
          GimbalSlider[i].setMin(0).setMax(12);
        }
        switch((int)GimbalSlider[i].getValue()) {
        case 0:
          GimbalSlider[i].setCaptionLabel("ROLL") ;
          break;
        case 1:
          GimbalSlider[i].setCaptionLabel("NICK") ;
          break;
        case 2:
          GimbalSlider[i].setCaptionLabel("YAW")  ;
          break;
        case 3:
    ...
    if (ServosActive) {
      ...
      for ( i=0; i<8; i++) {
        if (ServoSliderC[i].getValue() < BreakPoint ) {
          ServoSliderC[i].setMin(0).setMax(12);
        }
        switch((int)ServoSliderC[i].getValue()) {
        case 0:
          ServoSliderC[i].setCaptionLabel("ROLL") ;
          break;
        case 1:
          ServoSliderC[i].setCaptionLabel("NICK") ;
          break;
        case 2:
          ServoSliderC[i].setCaptionLabel("YAW")  ;
          break;
        case 3:
    ...
    if (toggleWrite) {
      ...
      for ( i=0; i<4; i++) {
        int q= (int)(confINF[i].getValue());
        payload.add(char (q % 256) );
        payload.add(char (q / 256)  );
      }

      // PermanentLog
      int nn= round(confINF[4].getValue()*10);
      ...
      // MagDec
      nn= round(confINF[6].getValue()*10);
      payload.add(char (nn - ((nn>>8)<<8) ));
      payload.add(char (nn>>8)); // h


      // VBatscale
      nn= round(VBat[0].getValue());
      payload.add(char (nn)); // i
      for ( i=1; i<4; i++) {
        int q= int(VBat[i].getValue()*10);
        payload.add(char (q));
      } // j,k,l
  ...
  graphics.pushMatrix();
  graphics.translate(60, 60, -60);
  graphics.directionalLight(200, 200, 200, 0, 0, -1);
  graphics.rotateX(PI/2);
  graphics.rotateZ(h);
  graphics.rotateX(b);
  graphics.rotateY(a);
  graphics.stroke(150, 255, 150);
  graphics.strokeWeight(0);
  graphics.sphere(size/3);
  graphics.strokeWeight(3);
  graphics.line(0, 0, 10, 0, -size-5, 10);
  graphics.line(0, -size-5, 10, +size/4, -size/2, 10);
  graphics.line(0, -size-5, 10, -size/4, -size/2, 10);
  graphics.stroke(255);
  int MotToggleMove=200;

  graphics.textFont(font12);
  if (toggleGimbal || toggleTrigger) { //if (multiType == GIMBAL)
    graphics.noLights();
    graphics.camera();
    graphics.popMatrix();
    graphics.text("GIMBAL", xMot, yMot+25);
    
    text("GIMBAL", xObj + 5, yObj + 16);
    ...
    graphics.line(-size, -size, 0, 0);
    graphics.line(+size, -size, 0, 0);
    graphics.line(0, +size, 0, 0);
    graphics.noLights();
    graphics.camera();
    graphics.popMatrix();
    
    text("TRICOPTER", xObj + 5, yObj + 16);
    ...
    text("QUADRICOPTER +", xObj + 5, yObj + 16);
    ...
    text("QUADRICOPTER X", xObj + 5, yObj + 16);
    ...
    drawMotor(+size, 0, byteMP[1], 'L');
    graphics.line(0-size, 0, 0, 0);
    graphics.line(0+size, 0, 0, 0);
    graphics.line(0, size*1.5, 0, 0);
    graphics.noLights();
    graphics.camera();
    graphics.popMatrix();
    
    text("BICOPTER", xObj + 5, yObj + 16);
    ...
    text("TRICOPTER Y6", xObj + 5, yObj + 16);
    ...
    graphics.line(-size, -0.55*size, 0, 0);
    graphics.line(size, -0.55*size, 0, 0);
    graphics.line(-size, +0.55*size, 0, 0);
    graphics.line(size, +0.55*size, 0, 0);
    graphics.line(0, +size, 0, 0);
    ...
    text("HEXACOPTER", xObj + 5, yObj + 16);
    ...
    graphics.line(0, 0, 1.8*size, size);
    graphics.line(1.8*size, size, 1.8*size, size-30);
    graphics.line(1.8*size, size-30, 0, -1.5*size);
    graphics.line(0, 0, -1.8*size, +size);
    graphics.line(-1.8*size, size, -1.8*size, +size-30);
    graphics.line(-1.8*size, size-30, 0, -1.5*size);
    graphics.noLights();
    ...
    text("FLYING WING", xObj + 5, yObj + 16);
    ...
    graphics.line(+size, +1.2*size, +size, -1.2*size);
    graphics.line(-size, +1.2*size, -size, -1.2*size);
    graphics.line(+size, 0, 0, 0);
    graphics.line(-size, 0, 0, 0);
    graphics.noLights();
    graphics.camera();
    graphics.popMatrix();
    
    text("HEXACOPTER H", xObj + 5, yObj + 16);
    ...
    graphics.line(-0.55*size, -size, 0, 0);
    graphics.line(-0.55*size, size, 0, 0);
    graphics.line(+0.55*size, -size, 0, 0);
    graphics.line(+0.55*size, size, 0, 0);
    graphics.line(+size, 0, 0, 0);
    ...
    text("HEXACOPTER X", xObj + 5, yObj + 16);
    ...
    text("OCTOCOPTER X", xObj + 5, yObj + 16);
    ...
    text("OCTOCOPTER P", xObj + 5, yObj + 16);
    ...
    // Wing
    graphics.line(0, 0, Span, 0);
    graphics.line(Span, 0, Span, VingRoot);
    graphics.line(Span, VingRoot, 0, VingRoot);
    graphics.line(0, 0, -Span, 0);
    graphics.line(-Span, 0, -Span, VingRoot);
    graphics.line(-Span, VingRoot, 0, VingRoot);
    // Stab
    graphics.line(-(size*0.4), size, (size*0.4), size);
    graphics.line(-(size*0.4), size+5, (size*0.4), size+5);
    graphics.line(-(size*0.4), size, -(size*0.4), size+5);
    graphics.line((size*0.4), size, (size*0.4), size+5);
    // Body
    graphics.line(-2, size, -2, -size+5);
    graphics.line(2, size, 2, -size+5);
    graphics.line( -2, -size+5, 2, -size+5);
    // Fin
    graphics.line(0, size-3, 0, 0, size, 15);
    graphics.line(0, size, 15, 0, size+5, 15);
    graphics.line(0, size+5, 15, 0, size+5, 0);
    graphics.noLights();
    graphics.textFont(font12);
    graphics.camera();
    graphics.popMatrix();
    
    text("AIRPLANE", xObj + 5, yObj + 16);
    ...
    // Rotor
    graphics.ellipse(0, 0, 2*scalesize, 2*scalesize);
    // Body
    graphics.line(0, 1.5*scalesize, -5, -2, -0.5*scalesize, -5);
    graphics.line(0, 1.5*scalesize, -5, 2, -0.5*scalesize, -5);
    graphics.line( -2, -0.5*scalesize, -5, 2, -0.5*scalesize, -5);
    // Fin
    float finpos = scalesize * 1.3;
    int HFin=0;
    int LFin=15;
    graphics.line(0, finpos-3, 0, 0, finpos+7, -LFin);
    graphics.line(0, finpos+7, -LFin, 0, finpos+10, -LFin);
    graphics.line(0, finpos+10, -LFin, 0, finpos+5, 0);
    graphics.line(0, finpos-3, 0, 0, finpos, HFin);
    graphics.line(0, finpos, HFin, 0, finpos+5, HFin);
    graphics.line(0, finpos+5, HFin, 0, finpos+5, 0);

    // Stab
    graphics.line(-(scalesize*0.3), scalesize, -5, (scalesize*0.3), scalesize, -5);
    graphics.line(-(scalesize*0.3), scalesize+3, -5, (scalesize*0.3), scalesize+3, -5);
    graphics.line(-(scalesize*0.3), scalesize, -5, -(scalesize*0.3), scalesize+3, -5);
    graphics.line((scalesize*0.3), scalesize, -5, (scalesize*0.3), scalesize+3, -5);

    graphics.noLights();
    graphics.textFont(font12);
    graphics.text("Heli 120 CCPM", -42, -50);
    graphics.camera();
    graphics.popMatrix();
    ...
    graphics.ellipse(0, 0, 2*scalesize, 2*scalesize);
    // Body
    graphics.line(0, 1.5*scalesize, -2, -0.5*scalesize);
    graphics.line(0, 1.5*scalesize, 2, -0.5*scalesize);
    graphics.line( -2, -0.5*scalesize, 2, -0.5*scalesize);
    // Fin
    float finpos = scalesize * 1.3;
    int HFin=5;
    int LFin=10;
    ...
    // Stab
    graphics.line(-(scalesize*0.3), scalesize, (scalesize*0.3), scalesize);
    graphics.line(-(scalesize*0.3), scalesize+3, (scalesize*0.3), scalesize+3);
    graphics.line(-(scalesize*0.3), scalesize, -(scalesize*0.3), scalesize+3);
    graphics.line((scalesize*0.3), scalesize, (scalesize*0.3), scalesize+3);

    graphics.noLights();
    graphics.textFont(font12);
    graphics.text("Heli 90", -16, -50);
    graphics.camera();
    graphics.popMatrix();
    ...
    graphics.noLights();
    graphics.textFont(font12);
    graphics.text("PPM to 8 servos", -40, -50);
    graphics.camera();
    graphics.popMatrix();
    int ind=-5;
    for (i=0; i<8; i++) {
    ...
    float VingRoot = Span*0.25;
    // Stab
    graphics.line(0, VingRoot, (size*0.4), size);
    graphics.line(-(size*0.4), size+5, (size*0.4), size+5);
    graphics.line(0, VingRoot, -(size*0.4), size);
    graphics.line(-(size*0.4), size, -(size*0.4), size+5);
    graphics.line((size*0.4), size, (size*0.4), size+5);
    ...
    graphics.line( -2, -size+5, 2, -size+5);
    // Fins
    graphics.line(0, VingRoot-3, 0, 0, size, 15);
    graphics.line(0, size, 15, 0, size+5, 15);
    graphics.line(0, size+5, 15, 0, size+5, 0);
    graphics.line(0, VingRoot-3, 0, 0, size, -15);
    graphics.line(0, size, -15, 0, size+5, -15);
    ...
    text("Dualcopter", xObj + 5, yObj + 16);
  ...
  clip(xObj, yObj + 16, 225, 210 - 16);
  image(graphics, xObj, yObj + 16, 225, 210 - 16);
  noClip();
  ...
  stroke(0, 0, 255);
  if (azGraph) {
    if (scaleSlider.getValue()<2) {
      g_graph.drawLine(accYAW, -1000, +1000);
    } else {
      g_graph.drawLine(accYAW, 200*scaleSlider.getValue()-1000, 200*scaleSlider.getValue()+500);
    }
  }
  ...
  if (!hideDraw) {
    ...
    text("AUX1", xBox+55, yBox+5);
    text("AUX2", xBox+105, yBox+5);
    text("AUX3", xBox+165, yBox+5);
    text("AUX4", xBox+218, yBox+5);
    textFont(font8);
    text("LOW", xBox+37, yBox+15);
    text("MID", xBox+57, yBox+15);
    text("HIGH", xBox+74, yBox+15);
    text("L", xBox+100, yBox+15);
    text("M", xBox+118, yBox+15);
    text("H", xBox+136, yBox+15);
    text("L", xBox+154, yBox+15);
    text("M", xBox+174, yBox+15);
    text("H", xBox+194, yBox+15);
    text("L", xBox+212, yBox+15);
    text("M", xBox+232, yBox+15);
    text("H", xBox+252, yBox+15);
  }
  pushMatrix();
  translate(0, 0, 0);
  popMatrix();
...
void drawMotor(float x1, float y1, int mot_num, char dir) {   //Code by Danal
  ...
  if (dir == 'L') {
    d = +5;
    graphics.fill(254, 221, 44);
  }
  if (dir == 'R') {
    d = -5;
    graphics.fill(256, 152, 12);
  }
  graphics.ellipse(x1, y1, size, size);
  graphics.textFont(font15);
  graphics.textAlign(CENTER);
  graphics.fill(0, 0, 0);
  graphics.text(mot_num, x1, y1+5, 3);
  float y2 = y1-(size/2);
  graphics.stroke(255, 0, 0);
  graphics.line(x1, y2, 3, x1+d, y2-5, 3);
  graphics.line(x1, y2, 3, x1+d, y2+5, 3);
  graphics.line(x1, y2, 3, x1+d*2, y2, 3);
  graphics.popStyle();
}
